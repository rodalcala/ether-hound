...
const BlockDetails = ({ web3, getTransactionReceipt, activeTransaction, activeTransactionReceipt }) => {
  useEffect(() => {
    if (activeTransaction) getTransactionReceipt(activeTransaction.hash);
  },[getTransactionReceipt, activeTransaction]);

  if (!activeTransaction) return <Text>Transaction details are loading...</Text>
  return (
